...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (infoComic) {
    infoComic.innerHTML = `
      ...
      <p><strong>Autores:</strong> ${comic.autores.join(", ")}</p>
      <p><strong>Sinopsis:</strong> ${comic.sinopsis}</p>
      <p><strong>Género:</strong> ${comic.genero}</p>
      <p><strong>Personajes:</strong> ${comic.personajes.map(p => p.nombre).join(", ")}</p>
    `;
    ...
    comic.escenas.forEach((escena) => {
      ...
      miCard.innerHTML = `
        <a class="episode" href="#">
          <img src="${escena.image}" alt="${escena.nombre}" />
          <p>${escena.nombre}</p>
        </a>
      `;
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  }, 1500); // Tiempo que quieres que dure la intro
});
